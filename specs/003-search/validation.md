...
| Campos pesquisáveis | `searchRules` compara título, resumo e explicação | Aprovado por testes unitários |
| Caixa e acentos | Normalização Unicode e testes para `pontuacao` e `SAQUE` | Aprovado por testes unitários |
...
4. Abrir `/search?q=arremesso` e `/search` diretamente.
5. Navegar com `Tab` e conferir foco visível em celular e desktop no DevTools.

## Limitações conhecidas

- A busca considera somente regras básicas estáticas.
- Não há sinônimos, correção ortográfica, autocomplete ou busca no servidor.
