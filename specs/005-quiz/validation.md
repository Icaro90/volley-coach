...
| Cinco perguntas | `quizQuestions` possui cinco itens, protegidos por teste de integridade | Aprovado por testes unitários definidos |
...
| Pontuação e resultado | Reducer incrementa somente respostas corretas; `QuizPage` mostra resultado de `0` a `5` | Aprovado por testes de reducer e página |
...
| Teclado e responsividade | Radios, botões e foco global semântico existem; requer confirmação no navegador | Pendente de verificação manual |
...
| `npm run test` | Pendente de execução local | A sandbox bloqueia o processo interno usado pelo Vite/Vitest (`spawn EPERM`) antes de os testes iniciarem. |
| `npm run build` | Pendente de execução local | A sandbox não consegue carregar o binário nativo usado pelo Vite/Tailwind. |
...
5. Concluir as cinco perguntas e confirmar um resultado entre `0` e `5` e a mensagem de incentivo.
